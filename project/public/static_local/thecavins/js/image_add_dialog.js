	...
	image_template_html = $("#full_image_wrapper").html();
	    ...
	    // Properly redirect to the login page if the session expired
	    //var head = $(this).contents().find("head").html();
            //var temp_url = head.match(registrtation_re);
	    //if (temp_url) window.location.href = temp_url[1];
            ...
            if (image = results['uploaded_image']) {
                ...
                $("#crop_form").attr('action','/image/' + image['id'] + '/crop/');
